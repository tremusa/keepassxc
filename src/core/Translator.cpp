...
void Translator::installTranslators()
{
    ...
    for (const QString& path : paths) {
        if (!installTranslator(language, path) && !installTranslator("en_plurals", path)) {
...
/**
 * Install Qt base translator from system default path or from given local
 * search path if no qtbase translations were found at the system path.
 *
 * @param language translator language
 * @param path local search path
 * @return true on success
...
bool Translator::installQtTranslator(const QString& language, const QString& path)
{
    ...
    if (qtTranslator->load(QString("qtbase_%1").arg(language), QLibraryInfo::location(QLibraryInfo::TranslationsPath))) {
        return QCoreApplication::installTranslator(qtTranslator.take());
    } else if (qtTranslator->load(QString("qtbase_%1").arg(language), path)) {
        return QCoreApplication::installTranslator(qtTranslator.take());
    }
    return false;
}
